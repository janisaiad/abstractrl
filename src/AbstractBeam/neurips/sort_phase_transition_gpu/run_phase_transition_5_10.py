#!/usr/bin/env python3
"""Scan de transition de phase pour Sort(x) sur longueurs 5..10.

On augmente le nombre de swaps (mode nearly_sorted_swap) et on mesure le taux.
"""
...
def _run_suite(length: int, swaps: int, seed: int, port: int):
  set_global_seed(seed)
  rng = random.Random(seed)
  suite_name = f"len{length}_sw{swaps}"
...
def _extract_thresholds(rows):
  by_len = {}
  for r in rows:
    by_len.setdefault(r["length"], []).append(r)
  thresholds = {}
  for length, entries in by_len.items():
    entries = sorted(entries, key=lambda x: x["swaps"])
    first_not_full = next((e["swaps"] for e in entries if e["rate"] < 1.0), None)
    first_below_half = next((e["swaps"] for e in entries if e["rate"] < 0.5), None)
    thresholds[length] = {
        "first_not_full_success": first_not_full,
        "first_below_50pct": first_below_half,
    }
  return thresholds
...
def main():
  ...
  swaps_grid = [0, 2, 4, 6, 8, 10, 12, 16, 20]
  ...
  for length in lengths:
    for swaps in swaps_grid:
      ...
      row = _run_suite(length=length, swaps=swaps, seed=50000 + length * 100 + swaps, port=port)
      rows.append(row)
      port += 1
      print(f"DONE len={length}, swaps={swaps}: {row['solved']}/{row['total']} ({row['rate']*100:.1f}%)")

  thresholds = _extract_thresholds(rows)
  summary = {"grid": rows, "thresholds": thresholds}
  out_path = os.path.join(ROOT, "phase_transition_summary_5_10.json")
