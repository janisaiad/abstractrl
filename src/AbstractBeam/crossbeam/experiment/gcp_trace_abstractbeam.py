...
class GCGraph:
    def __init__(
        ...
        self.core_lb = max_core_lower_bound(self.adj_list)
        self.clique_lb = greedy_clique_lower_bound(self.adj_list, self.degrees)
        self.lower_bound = max(self.core_lb, self.clique_lb)
...
def max_core_lower_bound(adj_list: List[np.ndarray]) -> int:
    if not adj_list:
        return 0
    n = len(adj_list)
...
def greedy_k_assignment(graph: GCGraph, k: int, order: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    colors = np.zeros(n, dtype=np.int16)
    for v in order.tolist():
        penalties = np.zeros(k, dtype=np.int32)
        for u in graph.adj_list[v]:
            penalties[int(colors[u])] += 1
...
def global_feature_vector(graph: GCGraph, state: RepairState, metrics: StateMetrics, candidate_count: int) -> np.ndarray:
    ...
    x[21] = float(graph.core_lb) / max(state.k, 1)
        ...
        for _ in range(self.cfg.simulations):
            ...
            while True:
                ...
                if node.terminal or depth >= self.cfg.max_depth:
                    value = total_reward + (self.cfg.gamma ** depth) * leaf_value
                    break
                ...
                total_reward += (self.cfg.gamma ** depth) * reward
                node_id = child_id
                depth += 1
            backup = value
...
def solve_instance(
    ...
    if k is None:
        k = max(graph.lower_bound, graph.dsatur_ub)
