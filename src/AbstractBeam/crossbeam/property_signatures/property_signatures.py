...
def _property_signatures_domain_token():
  """Domain hint for VALUES_TO_TRY; import-safe (no sys.exit)."""
  token = os.environ.get("CROSSBEAM_PROPERTY_DOMAIN", "")
  if token:
    return token
  if len(sys.argv) >= 3:
    return sys.argv[2]
...
_argument = _property_signatures_domain_token()

if "dreamcoder" in _argument:
  VALUES_TO_TRY = {
    int: {
